...
try {
    ...
    if ($type === 'email') {
        ...
        // Accept either local number or combined country+number. If the value looks like a country+local number (e.g. 639112345678), use it as-is.
        $clean = preg_replace('/\D/', '', $value);
        // If the number is 10 digits (local), try to match common PH country code 63 as well.
        $candidates = [$clean];
        if (strlen($clean) === 10) {
            $candidates[] = '63' . $clean;
        }
        ...
        foreach ($candidates as $cand) {
            ...
            $stmt->execute([$cand]);
            if ($stmt->rowCount() > 0) { $found = true; break; }
        }
