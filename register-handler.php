...
// Normalize phone: expect local 10-digit mobile number (without leading zero), plus country code (e.g. 63)
if (empty($phone)) {
    $response['errors']['phone'] = 'Phone number is required';
} else {
    $clean = preg_replace('/\D/', '', $phone);
    // remove leading zero if provided
    if (strlen($clean) > 0 && $clean[0] === '0') $clean = substr($clean, 1);
    if (strlen($clean) !== 10) {
        $response['errors']['phone'] = 'Phone number must be exactly 10 digits (without leading zero)';
    }
    // normalize country code
    $countryCode = preg_replace('/\D/', '', $countryCode ?: '63');
    if ($countryCode === '') $countryCode = '63';
    $storePhone = $countryCode . $clean; // e.g. 639123456789
}
...
try {
    ...
    // Check if phone already exists (we store country+number, e.g. 639123...)
    $checkPhone = $pdo->prepare("SELECT janitor_id FROM janitors WHERE phone = ? LIMIT 1");
    $checkPhone->execute([$storePhone]);
    if ($checkPhone->rowCount() > 0) {
        $response['errors']['phone'] = 'This phone number is already registered';
        error_log("[v0] Phone already exists in janitors: $storePhone");
        echo json_encode($response);
        exit;
    }
