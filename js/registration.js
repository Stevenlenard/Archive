...
(function() {
  ...
  function initFormValidation() {
    ...
    // fixed country code for PH (no dropdown)
    ...
    function validatePhone(input) {
      ...
      // Remove leading zero if user included it accidentally
      if (clean.length > 0 && clean.charAt(0) === '0') clean = clean.slice(1);
      if (clean.length !== 10) { setInputError(input, err, 'Phone number must be exactly 10 digits (without leading zero)'); return false; }
      setInputSuccess(input, err); return true;
    }
    ...
    if (phoneInput) {
      phoneInput.addEventListener('blur', async () => {
        ...
        if (phoneInput.value.charAt(0) === '0') phoneInput.value = phoneInput.value.slice(1);
        if (!validatePhone(phoneInput)) return;
        // check uniqueness using combined country code + local number (PH +63)
        const combined = COUNTRY_CODE + phoneInput.value.trim();
        const data = await checkUnique('phone', combined);
        if (data && data.exists) {
...
function initRoleModal() {
  ...
  // Add click handlers to both login buttons
  if (signInLink) {
