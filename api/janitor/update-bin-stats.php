<?php
require_once '../../includes/config.php';
require_once __DIR__ . '/../../includes/sms_send.php';

if (!isJanitor()) {
    sendJSON(['success' => false, 'message' => 'Unauthorized']);
}

$janitor_id = getCurrentUserId();
$input = json_decode(file_get_contents('php://input'), true);

$bin_id = $input['bin_id'] ?? null;
$status = $input['status'] ?? null;
$action_type = $input['action_type'] ?? null;
$notes = $input['notes'] ?? null;

if (!$bin_id || !$status) {
    sendJSON(['success' => false, 'message' => 'Missing required fields']);
}

try {
    $pdo->beginTransaction();

    // Verify bin is assigned to janitor
    $stmt = $pdo->prepare("SELECT bin_id FROM bins WHERE bin_id = ? AND assigned_to = ?");
    $stmt->execute([$bin_id, $janitor_id]);
    if (!$stmt->fetch()) {
        sendJSON(['success' => false, 'message' => 'Bin not assigned to you']);
    }

    // Update bin status
    $stmt = $pdo->prepare("UPDATE bins SET status = ?, updated_at = NOW() WHERE bin_id = ?");
    $stmt->execute([$status, $bin_id]);

    // Record collection
    $stmt = $pdo->prepare("
        INSERT INTO collections (bin_id, janitor_id, action_type, status, notes, collected_at, completed_at)
        VALUES (?, ?, ?, 'completed', ?, NOW(), NOW())
    ");
    $stmt->execute([$bin_id, $janitor_id, $action_type, $notes]);

    // Record task completion
    $stmt = $pdo->prepare("
        UPDATE tasks 
        SET status = 'completed', completed_at = NOW()
        WHERE bin_id = ? AND janitor_id = ? AND status != 'completed'
        LIMIT 1
    ");
    $stmt->execute([$bin_id, $janitor_id]);

    $pdo->commit();

    // After successful commit, send SMS to assigned janitor when applicable
    try {
        $sendPhone = null;
        $bin_code = null;
        $location = null;
        $janitor_name = null;
        // get janitor phone (current janitor is the assignee), bin code, location, and janitor name
        $pstmt = $pdo->prepare("SELECT j.phone AS phone, j.first_name, j.last_name, b.bin_code AS bin_code, b.location AS location FROM bins b LEFT JOIN janitors j ON b.assigned_to = j.janitor_id WHERE b.bin_id = ? LIMIT 1");
        $pstmt->execute([(int)$bin_id]);
        $prow = $pstmt->fetch(PDO::FETCH_ASSOC);
        if ($prow) {
            $sendPhone = trim($prow['phone'] ?? '');
            $bin_code = $prow['bin_code'] ?? null;
            $location = $prow['location'] ?? null;
            $janitor_name = trim(($prow['first_name'] ?? '') . ' ' . ($prow['last_name'] ?? ''));
        }

        if (!empty($sendPhone) && in_array($status, ['full', 'empty'], true)) {
            $binDisplay = $bin_code ?? "#{$bin_id}";
            $locDisplay = $location ?? 'unknown location';
            $nameDisplay = !empty($janitor_name) ? $janitor_name : 'there';
            if ($status === 'full') {
                $smsMessage = "Hi {$nameDisplay}, please clean {$binDisplay} at {$locDisplay} immediately!";
            } else {
                $smsMessage = "Hi {$nameDisplay}, thank you for emptying the bin {$binDisplay} at {$locDisplay}";
            }
            $sres = sms_send($sendPhone, $smsMessage);
            if (empty($sres['success'])) {
                error_log('[sms_send] api/janitor/update-bin-stats.php failed for bin ' . intval($bin_id) . ': ' . json_encode($sres));
            }
        }
    } catch (Exception $e) {
        error_log('[sms_send] api/janitor/update-bin-stats.php exception: ' . $e->getMessage());
    }

    sendJSON(['success' => true, 'message' => 'Bin status updated successfully']);
} catch (Exception $e) {
    $pdo->rollBack();
    sendJSON(['success' => false, 'message' => $e->getMessage()]);
}
?>
