...
<head>
  ...
  <title>Smart Trashbin - Admin Forgot Password</title>
...
<body>
  ...
  <header class="header">
    <div class="header-container">
      ...
      <nav class="nav-buttons">
        <a href="admin-login.php" class="btn btn-login">
      ...
      <div class="forgot-password-form-section">
        <div class="form-container">
          ...
          <div id="step-email" class="step active">
            ...
            <div class="back-link">
              <p>Remember your password? <a href="admin-login.php">Sign in</a></p>
            </div>
          </div>
          ...
          <div id="step-otp" class="step">
            ...
            <div class="back-link">
              <p>Didn't receive code? <a href="#" id="resendLink" onclick="sendOtp(true); return false;">Resend OTP</a></p>
            </div>
          </div>
  ...
  <span class="btn-text"><i class="fas fa-lock"></i> Reset Password</span>
            ...
            <div class="back-link">
              <p>Password reset successful? <a href="admin-login.php">Sign in now</a></p>
            </div>
          </div>
        </div>
      </div>
...
async function sendOtp(isResend = false) {
  ...
  // Show loading state (handles initial send and resends)
  const btn = isResend ? document.getElementById('resendLink') : document.getElementById("btn-send-otp");
  ...
  try {
    const res = await fetch("admin-check-and-send-otp.php", {
      method: "POST",
    ...
    if (isResend && btn) {
      ...
      btn.innerHTML = originalBtnHtml || 'Resend OTP';
    } else if (btn) {
      btn.disabled = false;
...
async function verify() {
  ...
  try {
    const res = await fetch("admin-verify-otp.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
if (!pwRule.test(new_password)) {
  ...
  try {
    const res = await fetch("admin-reset-password.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
