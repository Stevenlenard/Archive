...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'toggle_active') {
            ...
            // Send SMS when reactivating (status becomes 'empty')
            try {
                if ($new_status === 'empty') {
                    $phone = null; $bin_code = null; $location = null; $janitor_name = null;
                    $q = $conn->prepare("SELECT b.bin_code, b.location, j.phone, CONCAT(j.first_name, ' ', j.last_name) AS janitor_name FROM bins b LEFT JOIN janitors j ON b.assigned_to = j.janitor_id WHERE b.bin_id = ? LIMIT 1");
                    if ($q) {
                        $q->bind_param('i', $bin_id);
                        $q->execute();
                        $res = $q->get_result();
                        if ($res && $row = $res->fetch_assoc()) {
                            ...
                            $janitor_name = trim($row['janitor_name'] ?? '');
                        }
                        $q->close();
                    }
                    ...
                    if (!empty($phone)) {
                        ...
                        $nameDisplay = $janitor_name ? $janitor_name : 'there';
                        ...
                        if (empty($sres['success'])) {
                            error_log('[sms_send][bins.php toggle_active] failed: ' . json_encode($sres));
                        }
                    }
                }
            } catch (Exception $e) {
                error_log('[sms_send][bins.php toggle_active] exception: ' . $e->getMessage());
            }
        ...
        if ($action === 'edit_status') {
            ...
            // Send SMS when status is 'full' or 'empty'
            try {
                if (in_array($status, ['full', 'empty'], true)) {
                    ...
                    if (!empty($phone)) {
                        ...
                        $nameDisplay = $janitor_name ? $janitor_name : 'there';
                        if ($status === 'full') {
                            $smsMessage = "Hi {$nameDisplay}, please clean Bin {$binDisplay} at {$locDisplay} immediately!";
                        } else {
                            $smsMessage = "Hi {$nameDisplay}, thank you for emptying the Bin {$binDisplay} at {$locDisplay}";
                        }
                        $sres = sms_send($phone, $smsMessage);
                        if (empty($sres['success'])) {
                            error_log('[sms_send][bins.php edit_status] failed: ' . json_encode($sres));
                        }
                    }
                }
            } catch (Exception $e) {
                error_log('[sms_send][bins.php edit_status] exception: ' . $e->getMessage());
            }
...
// Send notification (redirected to janitor_alerts)
if ($action === 'send_notification') {
    ...
    try {
        ...
        $res = $conn->query("SELECT assigned_to, bin_code FROM bins WHERE bin_id = " . $bin_id);
        $assigned = ($res && $r = $res->fetch_assoc()) ? $r['assigned_to'] : null;
        $binCode = ($r['bin_code'] ?? 'Unknown');

        if (!$assigned) {
            echo json_encode(['success' => false, 'error' => 'No janitor assigned']);
            exit;
        }

        $title = "Bin full: Bin " . $binCode;

        if (isset($pdo) && $pdo instanceof PDO) {
            $stmt = $pdo->prepare("
                INSERT INTO janitor_alerts (janitor_id, bin_id, title, message, is_read, created_at)
                VALUES (:janitor_id, :bin_id, :title, :message, 0, NOW())
            ");
            $stmt->execute([
                ':janitor_id' => $assigned,
                ':bin_id' => $bin_id,
            ...
            if ($conn->query("SHOW TABLES LIKE 'janitor_alerts'")->num_rows > 0) {
                $stmt = $conn->prepare("
                    INSERT INTO janitor_alerts (janitor_id, bin_id, title, message, is_read, created_at)
                    VALUES (?, ?, ?, ?, 0, NOW())
                ");
                if ($stmt) {
                    $stmt->bind_param("iiss", $assigned, $bin_id, $title, $message);
                    $stmt->execute();
                    $stmt->close();
                }
            }
        ...
        echo json_encode(['success' => false, 'error' => 'Failed to send alert']);
        exit;
    }
}
...
// AUTO-REFRESH EVERY 1 SECOND (REAL-TIME BIN STATUS)
