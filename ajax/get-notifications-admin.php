...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        // NEW: Bin cleaned event — create a notification that the assigned janitor cleaned a bin
        if ($action === 'bin_cleaned') {
            // Expected: bin_id and optionally janitor_id (if omitted, use current janitor)
            $bin_id = isset($_POST['bin_id']) && $_POST['bin_id'] !== '' ? intval($_POST['bin_id']) : null;
            if (!$bin_id) throw new Exception('Missing bin_id');

            // Determine janitor who cleaned: prefer provided janitor_id, else current user if janitor
            $janitor_id = isset($_POST['janitor_id']) && $_POST['janitor_id'] !== '' ? intval($_POST['janitor_id']) : null;
            ...
            // Lookup bin code and janitor name for message context
            $bin_code = null;
            $janitor_name = null;
            if (isset($pdo) && $pdo instanceof PDO) {
                // bin code
                $stmt = $pdo->prepare("SELECT bin_code FROM bins WHERE bin_id = ? LIMIT 1");
                $stmt->execute([$bin_id]);
                $b = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($b) $bin_code = $b['bin_code'] ?? null;
                // janitor name
                if ($janitor_id) {
                    $stmtJ = $pdo->prepare("SELECT CONCAT(first_name, ' ', last_name) AS name, email FROM janitors WHERE janitor_id = ? LIMIT 1");
                    $stmtJ->execute([$janitor_id]);
                    $jrow = $stmtJ->fetch(PDO::FETCH_ASSOC);
                    if ($jrow) $janitor_name = trim($jrow['name'] ?: ($jrow['email'] ?? 'Janitor #' . $janitor_id));
                }
            } else {
                $b = $conn->query("SELECT bin_code FROM bins WHERE bin_id = " . intval($bin_id) . " LIMIT 1");
                if ($b && $br = $b->fetch_assoc()) $bin_code = $br['bin_code'] ?? null;
                if ($janitor_id) {
                    if ($stmtJ = $conn->prepare("SELECT CONCAT(first_name, ' ', last_name) AS name, email FROM janitors WHERE janitor_id = ? LIMIT 1")) {
                        $stmtJ->bind_param("i", $janitor_id);
                        $stmtJ->execute();
                        $jr = $stmtJ->get_result()->fetch_assoc();
            ...
            // Insert notification for admins (admin_id NULL) with janitor and bin info, leave is_read = 0 so admins see it
            if (isset($pdo) && $pdo instanceof PDO) {
                $ins = $pdo->prepare("
        ...
        if ($action === 'delete') {
            ...
            echo json_encode(['success' => true, 'message' => 'Notification deleted', 'notification_id' => $id]);
            exit;
        }
...
<head>
  ...
  <style>
    /* Ensure notification action buttons render side-by-side and stay right-aligned */
    #notificationsTableBody td.text-end,
    table td.text-end,
    .table td.text-end {
    ...
    /* Make buttons inline and avoid full-width stacking */
    #notificationsTableBody td.text-end .btn,
    #notificationsTableBody td.text-end .text-muted.small {
              ...
              <tr data-id="<?php echo $nid !== null ? $nid : ''; ?>"
                ...
                <td class="text-end">
                  <?php if ($nid !== null && !$isRead): ?>
                    <button class="btn btn-sm btn-success mark-read-btn me-2" data-id="<?php echo $nid; ?>"><i class="fas fa-check me-1"></i>Read</button>
                    <button class="btn btn-sm btn-outline-danger delete-notif-btn" data-id="<?php echo $nid; ?>"><i class="fas fa-trash-alt me-1"></i>Delete</button>
                  <?php elseif ($nid !== null && $isRead): ?>
                    <span class="text-muted small">Read</span>
                    <button class="btn btn-sm btn-outline-danger ms-2 delete-notif-btn" data-id="<?php echo $nid; ?>"><i class="fas fa-trash-alt me-1"></i>Delete</button>
                  <?php else: ?>
                    <button class="btn btn-sm btn-success mark-read-btn me-2" data-id="" data-bin-id="<?php echo e($n['bin_id'] ?? ''); ?>" data-janitor-id="<?php echo e($n['janitor_id'] ?? ''); ?>" data-title="<?php echo e($n['title'] ?? ''); ?>" data-message="<?php echo e($n['message'] ?? ''); ?>"><i class="fas fa-check me-1"></i>Read</button>
                  <?php endif; ?>
                </td>
              </tr>
...
(function() {
  ...
  $(document).on('click', '.delete-notif-btn', function (e) {
    ...
    const $btn = $(this);
    const id = $btn.data('id');
    if (!id) return showToast('Cannot delete this notification', 'warning');
    if (!confirm('Delete this notification? This action cannot be undone.')) return;

    $btn.prop('disabled', true).text('Deleting...');

    $.post('notifications.php', { action: 'delete', notification_id: id }, function (resp) {
      if (resp && resp.success) {
        const $row = $btn.closest('tr');
        $row.fadeOut(200, function () { $(this).remove(); });
        showToast(resp.message || 'Notification deleted', 'success');
      } else {
        showToast((resp && resp.message) ? resp.message : 'Failed to delete notification', 'danger');
...
setInterval(function () {
    $.get('ajax/get-notifications-admin.php', function (response) {
        ...
        if (!response.notifications || response.notifications.length === 0) {
            tbody.html('<tr class="no-results"><td colspan="6" class="text-center py-4 text-muted">No notifications found</td></tr>');
            return;
        }
        ...
        response.notifications.forEach(n => {
            ...
            let time = n.created_at ? new Date(n.created_at).toLocaleString() : '-';
            let target = '-';
            let badgeClass = n.notification_type === 'critical' ? 'danger' : 
                            n.notification_type === 'warning' ? 'warning' : 
                            n.notification_type === 'success' ? 'success' : 'info';
            ...
            if (n.bin_id) {
                ...
                target = n.admin_name ? n.admin_name : "Admin #" + n.admin_id;
            }
            
            let notifId = n.admin_notification_id || n.notification_id || '';
            let actionHtml = '';
            if (notifId && n.is_read == 0) {
                actionHtml = `<button class="btn btn-sm btn-success mark-read-btn me-2" data-id="${notifId}">
                    <i class="fas fa-check me-1"></i>Read
                </button><button class="btn btn-sm btn-outline-danger delete-notif-btn" data-id="${notifId}"><i class="fas fa-trash-alt me-1"></i>Delete</button>`;
            } else if (notifId) {
                actionHtml = `<span class="text-muted small"><i class="fas fa-check-circle me-1"></i>Read</span> <button class="btn btn-sm btn-outline-danger ms-2 delete-notif-btn" data-id="${notifId}"><i class="fas fa-trash-alt me-1"></i>Delete</button>`;
            } else {
                actionHtml = `<button class="btn btn-sm btn-success mark-read-btn me-2" data-id="" data-bin-id="${n.bin_id || ''}" data-janitor-id="${n.janitor_id || ''}" data-title="${(n.title||'').replace(/\"/g,'&quot;')}" data-message="${(n.message||'').replace(/\"/g,'&quot;')}">
                    <i class="fas fa-check me-1"></i>Read
                </button>`;
            }
            
            tbody.append(`
                <tr class="${isRead}" data-id="${notifId}">
                    <td>${time}</td>
                    <td><span class="badge bg-${badgeClass}">${n.notification_type.charAt(0).toUpperCase() + n.notification_type.slice(1)}</span></td>
                    <td>${n.title}</td>
                    <td class="d-none d-md-table-cell"><small class="text-muted">${n.message}</small></td>
